...
<script type="text/javascript" >
	$(function () {
			...
			$.ajax({
				...
				success:function (data) {
					...
					$.each(data,function (i,n) {
							html+='<tr>';
							html+='<td><input type="radio" name="xzActivity" value="'+n.id+'"/></td>';
		...
		$("#commitActivityBtn").click(function () {
			var $xz = $("input[name=xzActivity]:checked");
			if($xz.length==0){
				alert("请选择市场活动源");
			}else {
				var aId = $xz.val();
				$("#create-source").val(aId);
				var name = $("#"+aId).html();
				$("#create-ActivitySource").val(name);
		...
		$("#searchConBtn").click(function () {
			$.ajax({
				...
				success:function (data) {
					...
					$.each(data,function (i,n) {
						html+='<tr>';
						html+='<td><input type="radio" name="xzContacts" value="'+n.id+'"/></td>';
		...
		// 提交联系人按钮的绑定事件
		$("#commitContactsBtn").click(function () {
			var $xz = $("input[name=xzContacts]:checked");
			if($xz.length==0){
				alert("请选择联系人");
			}else {
				var cId = $xz.val();
...
<%--							</tr>--%>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
					<button type="button" class="btn btn-primary" id="commitContactsBtn">提交</button>
				</div>
			</div>
		</div>
	</div>
	...
	<form class="form-horizontal" role="form" style="position: relative; top: -30px;" id="tranForm" action="workbench/transaction/add.do" method="post">
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" name="activityId" id="create-source">
			</div>
		</div>
		
		<div class="form-group">
			<label for="create-contactsName" class="col-sm-2 control-label">联系人名称&nbsp;&nbsp;<a href="javascript:void(0);" id="searchConBtn"><span class="glyphicon glyphicon-search"></span></a></label>
			<div class="col-sm-10" style="width: 300px;">
